...
public class ImMessageUtil {
    ...
    public void onEvent(MessageEvent event) {
        ...
        if (canShow) {
            ...
            //条件1 APP不在前台
            boolean notFrontGround = !AppConfig.getInstance().isFrontGround();
            //条件2 APP在前台时在聊天界面时聊天对象不是对方时
            boolean notUserBean = true;
            String chatId = SpUtil.getInstance().getStringValue(Constants.CHAT_STATUS);//10038849
            if (chatId.length() != 0) {
                String fromID = msg.getFromID();//mdsj_10038849
                if (fromID != null && fromID.contains("_")) {
                    String[] split = fromID.split("_");
                    if (split.length == 2 && split[1].equals(chatId)) {
                        notUserBean = false;
                    }
                }
            }
            if (notFrontGround || notUserBean) {
                ...
                imNotifyMsgEvent.setEvent(event);
